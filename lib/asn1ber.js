...
// If skipValidation is true, the oid doesn't have to start with ".1.3"
// and less than 2 digits in length.
function oidArray(oid, skipValidation) {
  var bytes, i, val;
...
// Counter64 type, 0x46
exports.encodeCounter64 = function (val) {
    return(encodeIntegerish(val, T.Counter));
}
...
// If skipValidation is true, there can be more than 4 octets
// with values greater than 255 (they are changed to 0 for the buffer)
exports.encodeIpAddress = function (address, skipValidation) {
    ...
    if (!skipValidation && octets.length !== 4) {
...
exports.encodeOid = function (oid, skipValidation) {
    var buf, bytes, bytesLength, i, len;

    // Get the encoded format of the OID.
    bytes = oidArray(oid, skipValidation);

    // Get the encoded format of the length
	bytesLength = (bytes) ? bytes.length : 0;
    len = lengthArray(bytesLength);

    // Fill in the buffer with type, length and OID data.
    buf = new Buffer(1 + bytesLength + len.length);
    buf[0] = T.ObjectIdentifier;
    for (i = 1; i < len.length + 1; i++) {
        buf[i] = len[i - 1];
    }
    for (i = len.length + 1; i < bytesLength + len.length + 1; i++) {
...
exports.parseOid = function (buf) {
    ...
    // first oid can only be {0,1,2} so if it's higher, add the value back to second oid
    if (oid[0] > 2) {
      oid[1] += 40 * (oid[0]-2);
      oid[0] = 2;
    }
