...
function encode(pkt, skipValidation) {
    ...
    pkt.pdu.varbinds.forEach(function (vb) {
		...
		if (vb.type === asn1ber.types.Null || vb.type === null) {
			...
			val = asn1ber.encodeIpAddress(vb.value, skipValidation);
			...
			val = asn1ber.encodeOid(vb.value, skipValidation);
		} else if (vb.type === asn1ber.types.Counter) {
			val = asn1ber.encodeCounter(vb.value);
		} else if (vb.type === asn1ber.types.Counter64) {
			val = asn1ber.encodeCounter64(vb.value);
